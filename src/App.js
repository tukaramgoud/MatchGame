...
class App extends Component {
  state = {
    count: 60,
    tabIdIs: 'FRUIT',
    imageId: Math.floor(Math.random() * imagesList.length),
    score: 0,
    resultTab: false,
  }
  ...
  changeSubImageId = id => {
    ...
    this.setState({imageId: Math.floor(Math.random() * imagesList.length)})
    if (id === imagesList[imageId].id) {
  ...
  render() {
    const {count, imageId, tabIdIs, score, resultTab} = this.state
